...
export default class EditOrdersComponent extends Component<Props> {
  ...
  makeDeleteOrder(history: RouterHistory) {
      return (id: UUID) => {
        const orders = this.props.orders.filter(order => order.id != id)
        this.props.updateOrders(orders)
        history.replace(`/${this.props.listid}/orders/`)
      }
  }
        ...
        (
          <Route render={({history}: ContextRouter) =>
            <div className="EditOrdersComponent"
              onClick={(e: SyntheticEvent<>) => e.target === e.currentTarget &&history.push(`/${this.props.listid}/`)}
            >
              <div className="EditOrdersComponent__window">
                ...
                <div className="EditOrdersComponent__window__body">
                  <Route path='/:listid/orders/:orderid' render={({history, match}: ContextRouter) =>
                    <div className="EditOrdersComponent__order">
                      <NullSafeEditOrderComponent listid={this.props.listid} orders={this.props.orders} orderid={match.params['orderid']}
                        categories={this.props.categories} updateOrder={this.updateOrder} deleteOrder={this.makeDeleteOrder(history)}
                      />
                    </div>
                  }/>
                  ...
                  <div className="EditOrdersComponent__orders">
                    ...
                    <Link to={`/${this.props.listid}/`} className="EditOrdersComponent__orders__back Button Button--padded">Back</Link>
                  </div>
                </div>

                <div className="EditOrdersComponent__window__footer">
                  <Link to={`/${this.props.listid}/`} className="Button Button--padded">Back</Link>
                </div>
...
function NullSafeEditOrderComponent(props: NullSafeEditOrderProps) {
  ...
  return (
    <Fragment>
    {order != null
      ? <EditOrderComponent listid={props.listid} order={order} categories={props.categories} updateOrder={props.updateOrder} deleteOrder={props.deleteOrder}/>
      : <Fragment>
          <p>Not found :(</p>
          <Link to={`/${props.listid}/orders`}>Back</Link>
        </Fragment>
    }
    </Fragment>
  )
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Link to={`/${this.props.listid}/orders`} className="EditOrderComponent__back Button Button--padded">Back</Link>
      </div>
    )
  }
}
